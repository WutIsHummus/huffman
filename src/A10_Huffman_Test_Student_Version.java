    ...
    private static boolean compareFiles(final File file1, final File file2) throws IOException {
        ...
        try {
            ...
            for (int len; (len = in1.read(buf1)) > 0;) {
                ...
                for (int i = 0; i < len; i++) {
                    if (buf1[i] != buf2[i]) {
                        System.out.println("BYTE MISMATCH at global offset: " +
                                ((long)totalBytesRead + i));
                        System.out.println("Expected byte: " + (buf2[i] & 0xFF));
                        System.out.println("Your byte:     " + (buf1[i] & 0xFF));
                        System.out.println("Dump around mismatch:");
                        for (int j = Math.max(0, i-10); j < Math.min(len, i+10); j++) {
                            System.out.printf("expected[%d] = %02X    got[%d] = %02X%n",
                                    j, (buf2[j] & 0xFF),
                                    j, (buf1[j] & 0xFF));
                        }
                        return false;
